# ========= Copyright 2023-2024 @ deepprinciple Inc. All Rights Reserved. =========
...
class QCRolePlaying(RolePlaying):
    def __init__(self, **kwargs):
        ...
        init_user_sys_msg, init_assistant_sys_msg = self._construct_qc_sys_msgs()
    ...
    def _construct_qc_sys_msgs(self):
        user_system_prompt = f"""
===== RULES OF USER =====
Never forget you are a user and I am a quantum chemistry assistant. Never flip roles! You will always instruct me. We share a common interest in collaborating to successfully complete computational chemistry tasks.
I must help you to complete computational chemistry tasks using various quantum chemistry software packages.
...
Please note that computational chemistry tasks may be very complicated. Do not attempt to solve the task in a single step. You must instruct me to find the answer step by step.
Here are some tips that will help you to give more valuable instructions about our task to me:
<tips>
- I have various tools to use, such as search toolkit, code execution toolkit, document relevant toolkit, terminal toolkit, quantum chemistry computation toolkit, etc. Thus, You must think how computational chemist will solve the task step-by-step, and give me instructions just like that. For example, one may first use search or internal knowledgeto get some initial information and the proposal of a computational workflow, then think about what available computational chemistry tools to use, and then give me instructions about how to use them.
- Although the task is complex, the answer does exist. If you can't find the answer using the current scheme, try to re-plan and use other ways to find the answer, e.g. using other tools 
...
===== RULES OF ASSISTANT =====
Never forget you are a quantum chemistry assistant and I am a user. Never flip roles! Never instruct me! You have to utilize your computational chemistry knowledge to solve the task I assigned.
We share a common interest in collaborating to successfully complete complex computational chemistry tasks.
...
You must leverage your knowledge of quantum chemistry software and methods, try your best to solve the problem, and explain your solutions.
...
- When a tool fails to run, or the code does not run correctly, never assume that it returns the correct result and continue to reason based on the assumption, because the assumed result cannot lead you to the 
correct answer. The right way is to think about the reason for the error and try again.
...
- Always verify your computational setup is appropriate for the chemical system
    ...
    def step(
        ...
        if "TASK_DONE" not in user_msg.content:
            ...
            If there are available computational chemistry tools and you want to call them, never say 'I will ...', but first call the tool and reply based on tool call's result, and tell me which tool you have called.
            """
            ...
            Include a summary of the computational methods used, the key results obtained, and any important insights about the chemical system.
            """
        ...
        if "TASK_DONE" not in user_msg.content:
            modified_assistant_msg.content += f"""\n
                Provide me with the next instruction and input (if needed) based on my response and our current computational chemistry task: <task>{self.task_prompt}</task>
                Before producing the final answer, please check whether I have verified the computational results using appropriate validation methods. If not, please remind me to do that.
                If I have suggested computational methods, remind me to consider the computational cost vs. accuracy tradeoff.
...
class QCGAIARolePlaying(QCRolePlaying):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def step(
        ...
        if "TASK_DONE" not in user_msg.content:
            ...
            - `analysis`: enclosed by <analysis> </analysis>, a detailed analysis of the computational chemistry results.
...
def run_society(
    ...
    chat_history = []
    init_prompt = """
    Now please give me instructions to solve over overall computational chemistry task step by step. If the task requires specific quantum chemistry calculations, please instruct me to use appropriate software and methods.
        """
